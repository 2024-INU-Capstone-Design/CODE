...
# 동영상 파일 불러오기
video_path_1 = "ball-tracking/cam1_30fps.mp4"  # 여기에 사용하고 싶은 동영상 경로 입력
video_path_2 = "ball-tracking/cam2_30fps.mp4"
...
def visualize_3d_point(point_1, point_2):    
    ...
    if dir_vector[1] == 0:
        ...
        t_intersect = (plane_y - point_1[1]) / dir_vector[1]
        intersection = point_1 + t_intersect * dir_vector

        # 사각형 안에 있는지 확인 (X-Z 범위 확인)
        if 0 <= intersection[0] <= 42 and 0 <= intersection[2] <= 100:
            print(f"passed at {intersection}")
        else:
            print(f"nope at {intersection}")
    ...
    line_points = point_1[:, np.newaxis] + dir_vector[:, np.newaxis] * t_vals
...
# 현서 시작2
def combined_visualization(point_1, point_2, bottom_of_strike_zone, top_of_strike_zone):
    # 방향 벡터
    dir_vector = point_2 - point_1
    ...
    # 시선 벡터 (두 카메라 -> point_1 & point_2)
    ax.plot([x1_start, point_1[0]], [y1_start, point_1[1]], [z1, point_1[2]], color='blue', linestyle='dotted')
    ax.plot([x2_start, point_1[0]], [y2_start, point_1[1]], [z2, point_1[2]], color='green', linestyle='dotted')
    ax.plot([x1_start, point_2[0]], [y1_start, point_2[1]], [z1, point_2[2]], color='blue', linestyle='dotted')
    ax.plot([x2_start, point_2[0]], [y2_start, point_2[1]], [z2, point_2[2]], color='green', linestyle='dotted')

    # 교차점 표시
    ax.scatter(point_1[0], point_1[1], point_1[2], color='purple', s=50, label='3D Point 1')
    ax.text(point_1[0] + 10, point_1[1] + 10, point_1[2] + 10, f"({point_1[0]:.2f}, {point_1[1]:.2f}, {point_1[2]:.2f})", color='purple')
    ax.scatter(point_2[0], point_2[1], point_2[2], color='purple', s=50, label='3D Point 2')
    ax.text(point_2[0] + 10, point_2[1] + 10, point_2[2] + 10, f"({point_2[0]:.2f}, {point_2[1]:.2f}, {point_2[2]:.2f})", color='purple')
    ...
    ax.set_ylabel('Y (cm)')
    ax.set_zlabel('Z (cm)')
    ax.set_title('Unified 3D Visualization')
    ax.set_xlim(-500, 500)
    ax.set_ylim(-500, 500)
    ax.set_zlim(-500, 500)
    ax.legend()
...
# 현서 시작
def check_line_intersection_with_plane_and_plot(point1, point2):
    # 방향 벡터
    ...
    if dir_vector[1] == 0:
        ...
        t_intersect = (plane_y - point1[1]) / dir_vector[1]
        intersection = point1 + t_intersect * dir_vector
    ...
    # 직선 그리기
    t_vals = np.linspace(-1, 2, 100)
    line_points = point1[:, np.newaxis] + dir_vector[:, np.newaxis] * t_vals
    ax.plot(line_points[0], line_points[1], line_points[2], label='Line', color='blue')
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('3D Line and Plane Check')
