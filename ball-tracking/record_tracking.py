...
while cap.isOpened():
    ...
    _, roi_thresh = cv2.threshold(blurred, 8, 255, cv2.THRESH_BINARY)
    ...
    if flag2 == 1 and 0 <= predict_x < width and 0 <= predict_y < height:
    ...
    if updata_roi  and 0 <= predicted_cx < width and 0 <= predicted_cy < height:
