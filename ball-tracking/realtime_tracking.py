...
# 비디오 파일 불러오기
#video_path = 'C:/Users/ASUS/OneDrive/바탕 화면/realtime/ipcam_subincam_baseball_tracking.mp4'  # 여기에서 테스트할 비디오 파일 경로를 입력하세요.
video_path = 'C:/Users/ASUS/OneDrive/바탕 화면/realtime/ipcam_subincam_baseball_tracking.mp4'
...
# my 캠
initial_roi_x, initial_roi_y, initial_roi_width, initial_roi_height = 0, 50, 200, 300

# 현서 캠
#initial_roi_x, initial_roi_y, initial_roi_width, initial_roi_height = width - 200, 50, 200, 200
...
while cap.isOpened():
    ...
    blurred = cv2.GaussianBlur(gray_diff, (1, 1), 0)
    _, roi_thresh = cv2.threshold(blurred, 4, 255, cv2.THRESH_BINARY)
    ...
    if updata_roi:
      roi_x = max(0, min(predicted_cx - 150, width - roi_width))
      roi_y = max(0, min(predicted_cy - 150, height - roi_height))
      roi_width, roi_height = 300, 300
      
    elif updata_roi == False and flag3 == 1:
        # 공이 검출되지 않으면 ROI 위치는 고정, 너비만 증가
        roi_x = max(0, roi_x - 10)
        roi_width = min(width - roi_x, roi_width + 20)
