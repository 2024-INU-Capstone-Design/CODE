...
def visualize_3d_point(point_3d, left_point, right_point, degree = 2):     
    ...
    ax.grid(True)
    # ==== 비율 고정 ====
    ax.set_box_aspect([1, 1, 1])  # x:y:z 동일 스케일 적용
    plt.tight_layout()
    plt.show()
...
def calculate_strike_zone(height):
    top = float(height * 0.5635)
    bottom = float(height * 0.2764)
    return top, bottom

# 팀별 타자 키 리스트
blue_team_heights = [178, 180, 178, 170, 185, 186, 179, 180, 182]
white_team_heights = [180, 170, 175, 184, 182, 175, 185, 182, 170]
...
def set_strike_zone():
    global blue_index, white_index

    if current_team == 'blue':
        if blue_index < len(blue_team_heights):
            height = blue_team_heights[blue_index]
            print(f"[청팀] 타자 {blue_index + 1}번 키: {height}cm")
        else:
            print("청팀: 남은 타자가 없습니다.")
            return
    else:
        if white_index < len(white_team_heights):
            height = white_team_heights[white_index]
            print(f"[백팀] 타자 {white_index + 1}번 키: {height}cm")
        else:
            
            print("백팀: 남은 타자가 없습니다.")
            return

    top, bottom = calculate_strike_zone(height)
    print(f"  - 스트라이크존 위: {top:.2f}cm")
    print(f"  - 스트라이크존 아래: {bottom:.2f}cm")
...
while True:
    ...
    # 초기화: 각 팀 1번 타자로 세팅
        if current_team == 'blue':
            blue_index = 0
            print("청팀 스트라이크존 초기화: 1번 타자")
            set_strike_zone()
        else:
            white_index = 0
            print("백팀 스트라이크존 초기화: 1번 타자")
            set_strike_zone()


    elif key == ord('1'):
        current_team = 'blue'        
        print(f"청팀으로 전환되었습니다. 현재 타자: {blue_index + 1}번")
        set_strike_zone()

    elif key == ord('2'):
        current_team = 'white'
        print(f"백팀으로 전환되었습니다. 현재 타자: {white_index + 1}번")
        set_strike_zone()

    elif key == ord('.'):
        if current_team == 'blue':
            if len(blue_team_heights) == 0:
                print("청팀: 타자 목록이 비어 있습니다.")
            else:
                blue_index = (blue_index + 1) % len(blue_team_heights)
                print(f"청팀: 다음 타자 {blue_index + 1}번")
                set_strike_zone()
        else:
            if len(white_team_heights) == 0:
                print("백팀: 타자 목록이 비어 있습니다.")
            else:
                white_index = (white_index + 1) % len(white_team_heights)
                print(f"백팀: 다음 타자 {white_index + 1}번")
                set_strike_zone()
